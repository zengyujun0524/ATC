...
/*				if(3 == tableStatus) {
					log.info("----------如果平板状态为正在通话，则更改平板、无绳子机通话状态--------");
					Integer inr = userMapper.updateNoRopeMachineCallStatus(macAddress, 1);
					if(inr > 0) {
...
//	}
	 ...
	 */
	@Transactional(propagation=Propagation.SUPPORTS)
	public ResponseResult insertEquipment(Equipment equipment, Integer appTypeId,String pushToken,Integer sysType,String proNum) throws Exception {
		// 根据传入mac地址查询，是否已注册
		...
		if (index > 0) {
			...
			// 注册平板用户
			log.info("----------------------------》 注册平板用户");
			log.info("---MacAddress------>"+equipment.getMacAddress()+"----appTypeId----->"+appTypeId+"-------》 注册平板用户");
