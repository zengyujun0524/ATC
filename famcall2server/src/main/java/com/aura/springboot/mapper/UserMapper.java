...
public interface UserMapper {
	//根据mac地址 修改登录状态
	Integer updateLogsByMac(@Param("macAddress")String macAddress,@Param("logStatus")Integer logStatus) throws Exception;
	
	//根据im账号查询用户实体
	User selectUserByIm(@Param("imConnect")String imConnect)throws Exception; 
	 
	//修改登录状态
	Integer updateLogStatusByUserId(@Param("userId")Integer userId,@Param("logStatus")Integer logStatus) throws Exception;
	...
	//更新无绳子机名称
	Integer updateNoRopeMachineName(@Param("macAddress")String macAddress,@Param("username")String username,@Param("phone")String phone) throws Exception;
	
	//插入无绳子机
	@Lock(value = LockModeType.PESSIMISTIC_READ)
	Integer insertNoRopeMachine(@Param("macAddress")String macAddress,@Param("username")String username,@Param("phone")String phone,@Param("createTime")String createTime ,@Param("logStatus")Integer logStatus) throws Exception;
	...
	//查询平板关联的无绳子机是否已经绑定
	@Lock(value = LockModeType.PESSIMISTIC_READ)
	Integer selectNoRopeMachineCount(@Param("macAddress")String macAddress,@Param("phone")String phone) throws Exception;
	...
	//根据手机号码查询信息
	User selectUserByPhone(@Param("phone")String phone,@Param("countryCode")String countryCode) throws Exception;
	...
	//通过传入手机 imConnect 查找出对应的userId，然后通过userId在设备绑定表中查找对应的具有绑定关系的设备，以此查找出他们的imConnect 返回数据，用于Im推送消息
	...
	//修改所有无绳子机通话状态
	Integer updateNoRopeMachineCallStatus(@Param("macAddress")String macAddress,@Param("callStatus")Integer callStatus);
	
	//修改单个无绳子机通话状态
	Integer updateSingNoRopeMachineCallStatus(@Param("macAddress")String macAddress,@Param("phone")String phone,@Param("callStatus")Integer callStatus);
