package com.aura.springboot.utils.sms.httpclient;
...
public class ProxyHTTPClient implements HTTPClient {
	...
	public HTTPResponse fetch(HTTPRequest request) throws IOException, URISyntaxException {
		...
		// Build request
		RequestConfig reqConfig = RequestConfig.custom().setSocketTimeout(5 * 60 * 1000)
				.setConnectTimeout(request.connectTimeout).setConnectionRequestTimeout(request.requestTimeout)
				.setProxy(this.proxy).build();

		RequestBuilder reqBuilder = RequestBuilder.create(request.method.name()).setUri(uriBuilder.build())
				.setEntity(new ByteArrayEntity(request.body.getBytes(request.bodyCharset))).setConfig(reqConfig);
		...
		try {
			...
			try {
				...
				HTTPResponse res = new HTTPResponse().setRequest(request)
