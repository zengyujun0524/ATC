...
//			InputStream is = Apns4jDemo.class.getClassLoader().getResourceAsStream("Push_Dev.p12");
			...
			InputStream is = null;
			try {
				Resource resources = resolver.getResource("FamCall2PushCer20190103.p12");
...
//				is = getClassLoader().getResourceAsStream("Push_Dev.p12"); 
	 ...
	 */
	public void pushMsgiOS(String token, String callInNum, int ringTyte, int news, int control) {
		...
		log.info("--------消息数量-------- news" + news);

		payload.setBadge(news);
//		payload.addParam("callInNum", callInNum);  刚刚在忙   我是在耍猴吧 
