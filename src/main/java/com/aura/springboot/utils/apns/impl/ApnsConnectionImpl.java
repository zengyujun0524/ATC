...
import com.aura.springboot.utils.apns.model.Command;
...
//	private Log logger = LogFactory.getLog(ApnsConnectionImpl.class);
	private static final String TAG = ApnsConnectionImpl.class.getSimpleName().toString();
...
//				Log.e(TAG, "Payload execeed limit, the maximum size allowed is 256 bytes. " + payload);
				return;
			}
...
//					Log.e(TAG, connName + " " + e.getMessage(), e);
			...
			if (!isSuccessful) {
//				Log.e(TAG, String.format("%s Notification send failed. %s", connName, notification));
				return;
			} else {
//				Log.i(TAG, String.format("%s Send success. count: %s, notificaion: %s", connName,
				...
				try {
					...
					while (true) {
						try {
							...
							flag ++;
//							Log.e(TAG, "SocketTimeoutException=>"+e.toString()+"  flag:"+flag);
						}
					}
...
//						}
						...
						synchronized (lock) {
							...
							if (!found) {
//								Log.w(TAG, connName + " Didn't find error-notification in the queue. Maybe it's time to adjust cache length. id: " + errorId);
...
//						Log.e(TAG, connName + " Unexpected command or size. commend: " + command + " , size: " + size);
					}
				} catch (Exception e) {
//					Log.e(TAG, connName + " " + e.getMessage(), e);
				} finally {
