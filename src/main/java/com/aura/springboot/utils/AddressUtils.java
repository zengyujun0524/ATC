...
public class AddressUtils {
	...
	public static void main(String[] something_to_do) {
		String cary = intToIp(1980410048);
	 ...
	 * !"unknown".equalsIgnoreCase(ip)) { // 多次反向代理后会有多个IP值，第一个为真实IP。 int index =
	 * ip.indexOf(','); if (index != -1) { return ip.substring(0, index); } else {
	 ...
	 * 解析ip地址
	 *
	 * 设置访问地址为http://ip.taobao.com/service/getIpInfo.php 设置请求参数为ip=[已经获得的ip地址]
	 * 设置解码方式为UTF-8
	 *
	...
	public String getAddresses(String content, String encoding) throws UnsupportedEncodingException {
		...
		if (returnStr != null) {
			...
			return new StringBuffer("地址为：" + country + ",").append(region + "省,").append(city + "市,")
					.append(county + ",").append("ISP公司：" + isp).toString();
		}
		return null;
	}
